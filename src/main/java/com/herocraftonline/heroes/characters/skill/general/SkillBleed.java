...
public class SkillBleed extends TargettedSkill {
    ...
    public SkillBleed(final Heroes plugin) {
        super(plugin, "Bleed");
    ...
    public void init() {
        super.init();
        this.applyText = SkillConfigManager.getRaw(this, SkillSetting.APPLY_TEXT.node(), "%target% is bleeding!").replace("%target%", "$1").replace("$target$", "$1");
        this.expireText = SkillConfigManager.getRaw(this, SkillSetting.EXPIRE_TEXT.node(), "%target% has stopped bleeding!").replace("%target%", "$1").replace("$target$", "$1");
    }
