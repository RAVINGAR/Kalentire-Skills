...
public class SkillEntangle extends TargettedSkill {
    ...
    public SkillEntangle(final Heroes plugin) {
        // Heroes stuff
        super(plugin, "Entangle");
        setDescription("Roots your target in place for $1 second(s). The effect breaks when the target takes damage.");
    ...
    public String getDescription(final Hero hero) {
        final double duration = SkillConfigManager.getUseSetting(hero, this, SkillSetting.DURATION, 3000, false);
    ...
    public void init() {
        ...
        applyText = SkillConfigManager.getRaw(this, SkillSetting.APPLY_TEXT, ChatComponents.GENERIC_SKILL + "%target% has been rooted!").replace("%target%", "$1").replace("$target$", "$1");
        expireText = SkillConfigManager.getRaw(this, SkillSetting.EXPIRE_TEXT, ChatComponents.GENERIC_SKILL + "%target% has broken free from the root!").replace("%target%", "$1").replace("$target$", "$1");
    }
    ...
    public SkillResult use(final Hero hero, final LivingEntity target, final String[] args) {
        ...
        final int period = SkillConfigManager.getUseSetting(hero, this, SkillSetting.PERIOD, 100, false);

        //EntangleEffect EntangleEffect = new EntangleEffect(this, hero.getPlayer(), duration);
        final RootEffect rootEffect = new RootEffect(this, player, period, duration, applyText, expireText);
        ...
        final List<Location> particleLocations = GeometryUtil.circle(player.getLocation(), 36, 1.5);
        for (final Location particleLocation : particleLocations) {
            ...
            player.getWorld().spawnParticle(Particle.BLOCK_CRACK, particleLocation, 1, 0, 0.1, 0, Bukkit.createBlockData(Material.OAK_WOOD));
        }
