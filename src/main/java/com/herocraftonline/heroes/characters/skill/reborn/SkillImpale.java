...
public class SkillImpale extends TargettedSkill {
    ...
    public SkillImpale(final Heroes plugin) {
        super(plugin, "Impale");
    ...
    public String getDescription(final Hero hero) {
        ...
        final int stunDuration = SkillConfigManager.getUseSetting(hero, this, "pinned-stun-duration", 2000, false);
    ...
    public SkillResult use(final Hero hero, final LivingEntity target, final String[] args) {
        ...
        final Vector playerDirection = player.getLocation().getDirection().normalize().multiply(0.9);

        final Location behindLoc = target.getEyeLocation().add(playerDirection);
//        Location behindDoubleLoc = target.getEyeLocation().add(playerDirection.multiply(2));

        // get block
        final Material blockBehindTarget = behindLoc.getBlock().getType();
//        Material blockDoubleBehindTarget = behindDoubleLoc.getBlock().getType();
        ...
        if ((blockBehindTarget.isSolid() && !Util.transparentBlocks.contains(blockBehindTarget))) {
            ...
            final StunEffect effect = new StunEffect(this, player, duration, stunApplyText, expireText);
            ...
            final SlowEffect effect = new SlowEffect(this, player, duration, amplitude, applyText, expireText);
            plugin.getCharacterManager().getCharacter(target).addEffect(effect);
        }
