...
public class SkillShield extends PassiveSkill {
    ...
    private final ArrayList<Material> shieldItems = new ArrayList<>();

    public SkillShield(final Heroes plugin) {
        super(plugin, "Shield");
    ...
    public class CustomListener implements Listener {
        ...
        public CustomListener(final Skill skill) {
            this.skill = skill;
        }
        ...
        public void onWeaponDamage(final WeaponDamageEvent event) {
            ...
            if (hero.hasEffect(getName())) {
                final Material type = NMSHandler.getInterface().getItemInOffHand(player.getInventory()).getType();
                double multiplier = 1;
                switch (type) {
                    case IRON_DOOR:
