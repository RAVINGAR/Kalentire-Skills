...
public class SkillCripple extends TargettedSkill
{
    ...
    public String getDescription(Hero hero) {
        double baseDuration = SkillConfigManager.getUseSetting(hero, this, SkillSetting.DURATION.node(), 10000, false);
        ...
        int baseDamage = SkillConfigManager.getUseSetting(hero, this, SkillSetting.DAMAGE.node(), 5, false);
        ...
        int baseDamageTick = SkillConfigManager.getUseSetting(hero, this, SkillSetting.DAMAGE_TICK.node(), 2, false);
        ...
        int baseMaxDistance = SkillConfigManager.getUseSetting(hero, this, SkillSetting.MAX_DISTANCE.node(), 2, false);
        double maxDistanceIncreaseFactor = SkillConfigManager.getUseSetting(hero, this, SkillSetting.MAX_DISTANCE_INCREASE.node(), 0.0D, false);
        ...
        int baseCooldown = SkillConfigManager.getUseSetting(hero, this, SkillSetting.COOLDOWN.node(), 0, false);
        int cooldownReductionFactor = SkillConfigManager.getUseSetting(hero, this, SkillSetting.COOLDOWN_REDUCE.node(), 0, false);
        int cooldown = (baseCooldown - cooldownReductionFactor * hero.getHeroLevel(this)) / 1000;
        if (cooldown > 0) {
            description = description + " CD:" + cooldown + "s";
        }

        int baseManaCost = SkillConfigManager.getUseSetting(hero, this, SkillSetting.MANA.node(), 10, false);
        int manaReductionFactor = SkillConfigManager.getUseSetting(hero, this, SkillSetting.MANA_REDUCE.node(), 0, false);
        int mana = baseManaCost - manaReductionFactor * hero.getHeroLevel(this);
        if (mana > 0) {
            description = description + " M:" + mana;
        }

        int baseHealthCost = SkillConfigManager.getUseSetting(hero, this, SkillSetting.HEALTH_COST, 0, false);
        int healthCostReductionFactor = SkillConfigManager.getUseSetting(hero, this, SkillSetting.HEALTH_COST_REDUCE, mana, true);
        int healthCost = baseHealthCost - healthCostReductionFactor * hero.getHeroLevel(this);
        if (healthCost > 0) {
            description = description + " HP:" + healthCost;
        }

        int baseStaminaCost = SkillConfigManager.getUseSetting(hero, this, SkillSetting.STAMINA.node(), 0, false);
        int staminaCostReduction = SkillConfigManager.getUseSetting(hero, this, SkillSetting.STAMINA_REDUCE.node(), 0, false);
        int staminaCost = baseStaminaCost - staminaCostReduction * hero.getHeroLevel(this);
        if (staminaCost > 0) {
