...
//TODO make into passive
public class SkillEarthernFury extends TargettedSkill {

    private String applyText;
    private String expireText;

    public SkillEarthernFury(final Heroes plugin) {
    ...
    public void init() {
        ...
        applyText = SkillConfigManager.getRaw(this, SkillSetting.APPLY_TEXT, ChatComponents.GENERIC_SKILL + "%target% feels the earthern fury of %hero%!").replace("%target%", "$1").replace("$target$", "$1").replace("%hero%", "$2").replace("$hero$", "$2");
    ...
    public SkillResult use(final Hero hero, final LivingEntity target, final String[] args) {
        ...
        final int dexterityReduction = SkillConfigManager.getUseSetting(hero, this, "dexterity-reduction", 3, false);
        final int slownessAmplitude = SkillConfigManager.getUseSetting(hero, this, "slownessAmplitude", 2, false);
        final EarthernFuryDexterityEffect cEffect = new EarthernFuryDexterityEffect(this, hero, duration, dexterityReduction, slownessAmplitude, applyText, expireText);
        targCT.addEffect(cEffect);

        return SkillResult.NORMAL;
    }

    private static class EarthernFuryDexterityEffect extends AttributeDecreaseEffect {

        private BukkitTask effect;

        public EarthernFuryDexterityEffect(final SkillEarthernFury skill, final Hero applier, final long duration, final int decreaseValue, final int slownessAmplitude, final String applyText, final String expireText) {
        ...
        private void setEffect(final LivingEntity entity) {
            ...
            effect = new BukkitRunnable() {
                ...
                private final Location location = fEntity.getLocation();
