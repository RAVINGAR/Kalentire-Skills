...
public class SkillAccelerando extends ActiveSkill {
    ...
    public SkillResult use(Hero hero, String[] args) {
        ...
        for (int i = 0; i < circle(player.getLocation(), 72, radius).size(); i++) // for 1.13 (int i = 0; i < circle.size(); i++)
        {
            player.getWorld().spigot().playEffect(circle(player.getLocation(), 72, radius).get(i), Effect.NOTE, 0, 0, 0, 0.2F, 0, 1, 1, 20);
            //player.getWorld().spawnParticle(Particle.NOTE, circle.get(i), 1, 0, 0.2, 0, 1); this is 1.13
        }
        ...
        for (Hero tHero : hero.getParty().getMembers()) {
            ...
            //tHero.getPlayer().getWorld().spawnParticle(Particle.CLOUD, tHero.getPlayer().getLocation(), 16, 0, 0, 0, 1); 1.13 stuff
        }

        //remove this block for 1.13 compatibility
