...
public class SkillReborn extends PassiveSkill {
    ...
    public String getDescription(Hero hero) {
        final double health = ((SkillConfigManager.getUseSetting(hero, this, "health-percent-on-rebirth", 0.5, false) + (SkillConfigManager.getUseSetting(hero, this, "health-increase", 0.0, false) * hero.getSkillLevel(this))) * 100);
        final int cooldown = SkillConfigManager.getUseSetting(hero, this, SkillSetting.COOLDOWN.node(), 600000, false) + (SkillConfigManager.getUseSetting(hero, this, SkillSetting.COOLDOWN_REDUCE.node(), 0, false) * hero.getSkillLevel(this));
        ...
        public void onEntityDamage(EntityDamageEvent event) {
            ...
            if (hero.hasEffect("Reborn")) {
                if ((hero.getCooldown("Reborn") == null) || (hero.getCooldown("Reborn") <= System.currentTimeMillis())) {
                    final double regainPercent = SkillConfigManager.getUseSetting(hero, this.skill, "health-percent-on-rebirth", 0.5, false) + (SkillConfigManager.getUseSetting(hero, this.skill, "health-increase", 0.0, false) * hero.getSkillLevel(this.skill));
