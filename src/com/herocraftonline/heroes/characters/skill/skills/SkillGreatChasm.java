...
public class SkillGreatChasm extends SkillBaseBlockWave {
	...
	public SkillGreatChasm(Heroes plugin) {
		...
		setDescription("A chasm of chaos erupts and throws blocks around hitting all targets in a $1 degree arc in front of you, with a radius of $2, height of $3, and depth of $4, " +
				...
				"and slowing them for $8 second(s).");
	...
	public SkillResult use(final Hero hero, String[] strings) {
		...
		new BukkitRunnable() {
			...
			@Override
			public void run() {
				world.playSound(hero.getPlayer().getLocation(), Sound.ENTITY_ENDER_DRAGON_FLAP, volume, 1 - volume);
				world.playSound(hero.getPlayer().getLocation(), Sound.ENTITY_CAT_HISS, volume / 2, volume);
				world.playSound(hero.getPlayer().getLocation(), Sound.ENTITY_COW_STEP, 1 / volume, volume);
				volume -= 0.1;
