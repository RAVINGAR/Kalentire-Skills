...
public class SkillMarkShop extends ActiveSkill {
    ...
    public SkillMarkShop(Heroes plugin) {
        ...
        try {
            ...
            Heroes.log(Level.SEVERE, "SkillRecallShop: Could not get Residence or HeroTowns plugins! Region checking may not work!");
        }
    }
    ...
    public SkillResult use(Hero hero, String[] args) {
        ...
        ConfigurationSection skillSettings = hero.getSkillSettings("RecallShop");
        ...
        if (args.length > 0 && args[0].equalsIgnoreCase("reset")) {
            ...
            Messaging.send(player, "Your recallshop is currently marked on $1 at: $2, $3, $4", world.getName(), (int) xyzyp[0], (int) xyzyp[1], (int) xyzyp[2]);
            return SkillResult.SKIP_POST_USAGE;
        } else {
            // Save a new mark
            ...
            if (herotowns) {
                if (!ht.getGlobalRegionManager().canBuild(player, loc)) {
                    Messaging.send(player, "You cannot Mark Shop in a Town you have no access to!");
                    return SkillResult.FAIL;
                }
            }
            ...
            if (worldguard) {
                if (!wgp.canBuild(player, loc)) {
                    Messaging.send(player, "You cannot Mark Shop in a Region you have no access to!");
                    return SkillResult.FAIL;
                }
            }

            if (plugin.getServerName() != null) {
                hero.setSkillSetting("RecallShop", "server", plugin.getServerName());
            }
            hero.setSkillSetting("RecallShop", "world", loc.getWorld().getName());
            hero.setSkillSetting("RecallShop", "x", loc.getX());
            hero.setSkillSetting("RecallShop", "y", loc.getY());
            hero.setSkillSetting("RecallShop", "z", loc.getZ());
            hero.setSkillSetting("RecallShop", "yaw", (double) loc.getYaw());
            hero.setSkillSetting("RecallShop", "pitch", (double) loc.getPitch());
