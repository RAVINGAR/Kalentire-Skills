...
public class SkillForceFire extends SkillBaseSphere {
	...
	public SkillForceFire(Heroes plugin) {
		...
		setDescription("Call upon the force of fire to damage and knock back enemies within $1 blocks for $2 every $3 seconds for $4 seconds. $5 $6");
		...
		if (isAreaSphereApplied(hero)) {
			...
			applyAreaSphereEffect(hero, period, duration, radius, new SphereActions() {
				...
				public void sphereTickAction(Hero hero, AreaSphereEffect effect) {
					...
					hero.getPlayer().getWorld().playSound(hero.getPlayer().getLocation(), Sound.ENTITY_GHAST_SHOOT.value(), 0.25f, 0.000001f);
				}
