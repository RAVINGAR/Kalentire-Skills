...
public class SkillEarthquake extends ActiveSkill {
    ...
    public SkillEarthquake(Heroes plugin) {
        ...
        setDescription("You blast chunks of earth forward for $1 seconds, dealing $2 damage to $3enemies it hits.");
        setUsage("/skill earthquake");
        setArgumentRange(0, 0);
        setIdentifiers("skill earthquake");
        setTypes(SkillType.DAMAGING, SkillType.ABILITY_PROPERTY_MAGICAL, SkillType.ABILITY_PROPERTY_EARTH, SkillType.AGGRESSIVE);
    }
    ...
    public SkillResult use(final Hero hero, String[] strings) {
        ...
        EarthquakeEffect fbe = new EarthquakeEffect(em);
        fbe.setLocation(loc);
    ...
    private class EarthquakeEffect extends Effect {

        // Effect settings
        public ParticleEffect dirtParticle = ParticleEffect.BLOCK_CRACK;
        public ParticleData dirtData = new BlockData(Material.DIRT, (byte) 0);
        ...
        public EarthquakeEffect(EffectManager manager) {
            super(manager);
        ...
        public void onRun() {
            ...
            if (loc.getBlock().getType() != Material.AIR
                    && Block.getById(loc.getBlock().getTypeId()).getMaterial().isSolid()) {
                loc.add(0, 1, 0);
            }
            ...
            for (int i = 0; i < 3; i++) {
                ...
                for (Player player : casterPlayer.getWorld().getPlayers()) {
                    ...
                    sendPacket(player, new PacketPlayOutEntityEquipment(as.getId(), 4, CraftItemStack.asNMSCopy(new ItemStack(rand.nextInt(4) < 3 ? Material.GRASS : Material.DIRT))));
                }
