...
public class SkillSageTotem extends SkillBaseTotem implements Listener {
    ...
    public SkillSageTotem(Heroes plugin) {
        ...
        setDescription("Places a sage totem at target location that enlightens party members in a $1 radius, increasing Wisdom by $2. Lasts for $3 second(s).");
                ...
                new BukkitRunnable() {
                    ...
                    public void run() {
                        if (time < 1.0) {
                            ...
                            memberP.getWorld().spawnParticle(Particle.REDSTONE, location, 1, 0, 0, 0, 0.1, new Particle.DustOptions(Color.BLUE, 1));
                        } else {
                            ...
                            memberP.getWorld().spawnParticle(Particle.BLOCK_CRACK, location, 500, 0, 0, 0, 1, Bukkit.createBlockData(Material.LAPIS_BLOCK));
                            cancel();
                        }
                        time += 0.01;
                    }
    ...
    public ConfigurationSection getSpecificDefaultConfig(ConfigurationSection node) {
        node.set(SkillSetting.APPLY_TEXT.node(), ChatComponents.GENERIC_SKILL + "$1 is enlightened by the sage toem!");
        node.set(SkillSetting.EXPIRE_TEXT.node(), ChatComponents.GENERIC_SKILL + "$1 is unenlightened.");
    ...
    public String getApplyText() {
        return SkillConfigManager.getRaw(this, SkillSetting.APPLY_TEXT, ChatComponents.GENERIC_SKILL + "$1 is enlightened by the sage toem!");
    }

    public String getExpireText() {
        return SkillConfigManager.getRaw(this, SkillSetting.EXPIRE_TEXT, ChatComponents.GENERIC_SKILL + "$1 is unenlightened.");
    }
