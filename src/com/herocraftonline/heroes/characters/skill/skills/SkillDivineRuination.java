...
public class SkillDivineRuination extends SkillBaseBlockWave {
	...
	public SkillDivineRuination(Heroes plugin) {
		...
		setDescription("The purging quake erupts and throws blocks around hitting all targets in a $1 arc in front of you, with a radius of $2, height of $3, and depth of $4, " +
				"Expanding at a rate of $5 blocks per second. Targets can be hit hit a maximum of $6 times each hit dealing $7 damage, blinding them for $8 seconds.");
	...
	public SkillResult use(final Hero hero, String[] strings) {
		...
		castBlockWave(hero, hero.getPlayer().getLocation().getBlock(), new WaveTargetAction() {
			...
			public void onTarget(Hero hero, LivingEntity target, Location center) {
				if (damageCheck(hero.getPlayer(), target)) {
					...
					BlindEffect blind = new BlindEffect(SkillDivineRuination.this, hero.getPlayer(), duration);
		...
		new BukkitRunnable() {
			...
			@Override
			public void run() {
				world.playSound(hero.getPlayer().getLocation(), Sound.FIREWORK_BLAST, volume, 1f);
				volume -= 0.1;
