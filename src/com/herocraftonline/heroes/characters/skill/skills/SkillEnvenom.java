...
//http://pastie.org/private/qe2p3keym3evowadfefiq
...
public class SkillEnvenom extends ActiveSkill {
	...
	public SkillEnvenom(Heroes plugin) {
		...
		setDescription("Apply a poison to your weapon for $1 seconds. Attacking a target with your weapon will then poison them, causing $2 damage every $3 seconds, over $4 seconds.");
	...
	public String getDescription(Hero hero) {
		...
		return getDescription().replace("$1", buffDuration + "").replace("$2", damage + "").replace("$3", period + "").replace("$4", duration + "");
	}
}
