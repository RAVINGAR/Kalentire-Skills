...
//http://pastie.org/private/b2vjuhevcbwbdcxngmvhqw
...
public class SkillRoot extends TargettedSkill {
	...
	public SkillResult use(Hero hero, LivingEntity target, String[] args) {
		...
		RootEffect rootEffect = new RootEffect(this, period, duration, hero.getPlayer(), applyText, expireText);
	...
	private class RootEffect extends PeriodicExpirableEffect {
		...
		public RootEffect(Skill skill, int period, int duration, Player applier, String applyText, String expireText) {
		...
		public void tickMonster(Monster monster) {
			final Location location = monster.getEntity().getLocation();
			if ((location.getX() != loc.getX()) || (location.getZ() != loc.getZ())) {

				// Retain the monster's Y position
				loc.setY(location.getY());

				// Teleport the monster back into place.
				monster.getEntity().teleport(loc);
			}
